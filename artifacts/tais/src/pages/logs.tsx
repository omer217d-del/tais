      ...
      <div className="p-4 md:p-6 border-b border-border/50 sticky top-0 bg-background/95 backdrop-blur z-10 flex-shrink-0">
        ...
        {stats && (
          <div className="flex gap-2 text-xs overflow-x-auto pb-2 scrollbar-hide">
            ...
            <button onClick={() => setFilterLevel('warn')} className={`px-3 py-1 rounded border flex items-center gap-1 ${filterLevel === 'warn' ? 'bg-yellow-500/20 border-yellow-500 text-yellow-500' : 'bg-card border-border text-yellow-500 hover:bg-yellow-500/10'}`}>WARNINGS ({stats?.byLevel?.warn || 0})</button>
            <button onClick={() => setFilterLevel('info')} className={`px-3 py-1 rounded border flex items-center gap-1 ${filterLevel === 'info' ? 'bg-blue-400/20 border-blue-400 text-blue-400' : 'bg-card border-border text-blue-400 hover:bg-blue-400/10'}`}>INFO ({stats?.byLevel?.info || 0})</button>
          </div>
        )}
      </div>
